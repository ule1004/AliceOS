...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
TOTALSECTORCOUNT: dw 1			; 부트로더를 제외한 MINT64 OS 이미지의 크기
									; 최대 1152 섹터(0x90000byte)까지 가능
;	 1024 -> 1 16.06.01 Alice

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;	코드 영역
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
START:
	mov ax, 0x07C0					; 부트로더의 시작 어드레스(0x07C0)를 세그먼트 레지스터 값으로 변화
...
.SCREENCLEARLOOP:					; 화면을 지우는 루프
	...
	cmp si, 80 *25 * 2				; 화면의 전체 크기는 80문자 * 25라인 임
									; 출력한 문자의 수를 의미하는 SI레지스터와 비교
	jl .SCREENCLEARLOOP				; SI 레지스터가 80 * 25 * 2 보다 작다면 아직 이우지 못한 영역이 있으므로
...
RESETDISK:							; 디스크를 리셋하는 코드의 시작
	...
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	;	서비스 번호 80h, 드라이브 번호(80h=1st hard disk)
	mov ax, 0
	...
	mov si, 0x1000					; OS 이미지를 복사할 어드레스(0x1000)를
									; 세그먼트 레지스터 값으로 변환
	mov es, si						; ES 세그먼트 레지스터에 값 설정
	mov bx, 0x0000					; BX 레지스터에 0x0000을 설정하여 복사할
									; 어드레스를 0x1000:0000(0x1000)으로 최종 설정
...
READDATA:
	...
	;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	add si, 0x0020					; 512(0x200) 바이트만큼 읽었으므로, 이를 세그먼트 레지스터 값으로 변화
	mov es, si						; ES 세그먼트 레지스터에 더해서 어드레스를 한 섹터 만큼 증가

	;	한 섹터를 읽었으므로 섹터 번호를 증가시키고 마지막 세터(18)까지 읽었는지 판단
	;	마지막 섹터가 아니면 섹터 이동해서 다시 섹터 읽기 수행
	...
	;	마지막 섹터까지 밁었으면(섹터 번호가 19이면)헤드를 토글(0->1, 1->0)하고,
	;	섹터 번홓릏 1 증가
	xor byte [ HEADNUMBER ], 0x01	; 헤드 번호를 0x01과 XOR하여 토글(0->1, 1->1)
	mov byte [ SECTORNUMBER ], 0x01	; 섹터 번호를 다시 1로 섫정
...
PRINTMESSAGE:
	...
	mov ax, 0xB800					; 비디오 메모리 시작 어드레스(0xB800)를 세그먼트 레지스터 값으로 변환
	mov es, ax						; ES 세그먼트 레지스터에 설정
	...
	mov si, 160						; 한 라인의 바이트 수 (2*80 컬럼)를 SI 에지스터에 설정
	mul si							; AX 레지스터와 SI 에지스터를 곱하여 화면 Y 어드레스 계산
	mov di, ax						; 계싼된 화면 Y 어드레스를 DI 레지스터에 설정
...
.MESSAGELOOP						; 메시지를 출력하는 루프
	mov cl, byte [ si ]				; SI 레지스터가 가리키는 문자열 위치에서  문자를 CL 레지스터에 복사
									; CL 레지스터는 CX 레지스터의 하위 1바이트를 의미, 문자열은 1바이트면 충분하므로
									; CX 레지스터의 하위 1바이트만 사용

	cmp cl, 0						; 복사된 문자와 0을 비교
	je .MESSAGEEND					; 복사한 문자값이 0이면 문자열이 종료되었음을 의미하므로 .MESSAGEEND로 이동하여 문자 출력 종료

	mov byte [ es: di ], cl			; 0 이 아니라면 비디오 메모리 어드레스 0xB800:di 에 문자를 출력

	add si, 1						; SI 레지스터에 1을 더하여 다음 문자열로 이동
	add di, 2						; DI 레지스터에 2를 더하여 비디오 메모리의 다음 문자 위치로 이동
									; 비디오 메모리는 (문자, 속성)의 쌍으로 구성되므로 문자만 출력하면 2를 더해야함

	jmp .MESSAGELOOP				; 메시지 물력 루프 이동하여 다음 문자를 출력

.MESSAGEEND:						; 문자 출력 종료 루프
	pop dx							; 함수에서 사용이 끝난 DX 레지스터부터 ES 레지스터까지를 스택세
...
;	부트 로더 시작 메시지
MESSAGE1:
	db 'ALICE BOOTLOAD START~!!', 0	; 출력할 메시지의 정의
...
;	디스크 읽기에 관련된 변수들
SECTORNUMBER:
	db 0x02				; OS 이미지가 시작하는 섹터 번호를 저장하는 영역
HEADNUMBER:
	db 0x00				; OS 이미지가 시작하는 헤드 번호를 저장하는 영역
TRACKNUMBER:
	db 0x00				; OS 이미지가 시작하는 트랙 번호를 저장하는 영역

times 510 - ( $ - $$ ) db 0x00		; $:			현재라인의 어드레스
									; $$:			현재 섹셩(.text)의 시작 어드레스
									; $ - $$:		현재 섹션을 기준으로 하는 오프셋
									; 510 - ($-$$):	현재부터 어드레스 510까지
									; db 0x00:		1바이트를 선언하고 값은 0x00
									; time:			반복수행
									; 현재 위치에서 어드레스 510까지 0x00체움

db 0x55								; 1바이트를 선언하고 값은 0x55
db 0xAA								; 1바이트를 선언하고 값은 0xAA
									; 어드레스 511, 512 에 0x55, 0xAA를 써서 부트 섹터로 표기함
